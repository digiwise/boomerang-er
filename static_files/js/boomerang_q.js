...
boomerang.controller("EventsControl", function( $scope, $http, Config, $filter, $q ) {
    ...
    var resourcesCallback = false;
    $scope.qCalls = [];

    if (Config.enableEventResources) {
        $scope.eventResourcesCall = $http.jsonp(Config.resourcesURL).then(
          function(response){
            console.log('call1Response');
            console.log(response);
            $scope.qCalls.push($scope.eventResourcesCall);
          },
          function(err){
            console.log('call1Err --> '+err)
            $scope.qCalls = [];
          }
        );
    }

    $scope.eventsDetailsCall = $http.get("http://gdgfresno.com/gdgfeed.php?id="+Config.id);
    $scope.qCalls.push($scope.eventsDetailsCall);

    $q.all($scope.qCalls).
      then(function(values){
        console.log(values);
        if (values.length > 1) {
          if ("description" in values[0].data){
            data = values[0].data;
            resourcesCallback = values[1].data;
            //console.log('scriptcall --> '+resourcesCallback);
          }
          else if ("description" in values[1].data){
            resourcesCallback = values[0].data;
            data = values[1].data;
          }
        }
        else {
        ...
        for(var i=data.length-1;i>=0;i--){
          ...
          if (resourcesCallback){
             console.log('joshuaCall --> ' + resourcesCallback);
             var formatedDate = $filter('date')(data[i].start, 'yyyy-MM-dd');
             for(var iR=resourcesCallback.events.length-1;iR>=0;iR--){
                 console.log('formated --> '+formatedDate);
                 console.log('resourcesCallback --> '+resourcesCallback.events[iR].date);
                  if (formatedDate == resourcesCallback.events[iR].date){
                      console.log('matched');
                      data[i].resources = resourcesCallback.events[iR];

                  }
                  else {
                      data[i].resources = false;
                  }
